...
public class ReplicaManager1 {
	...
	static int count = 0;
	static int rCount = 1;
	static int countErrorMTL = 0;
	static int countErrorQUE = 0;
	static int countErrorSHE = 0;
	...
	static MTLServerError mtlServerError = new MTLServerError();
	static QUEServerError queServerError = new QUEServerError();
	static SHEServerError sheServerError = new SHEServerError();
	...
	public static void main(String args[]) throws IOException {
		...
		fileHandler = new FileHandler("C:/Users/gagan/Downloads/Replica Manager 3.txt");
		simpleFormatter=new SimpleFormatter();
		LOGGER.setUseParentHandlers(false);
		LOGGER.addHandler(fileHandler);
		fileHandler.setFormatter(simpleFormatter);
		LOGGER.info("Log File: Replica Manager 3");
		rm1.udpConnect();
	...
	public void udpConnect() {
		Runnable t1 = () -> {
			try {
				DatagramSocket udpSocket = new DatagramSocket(2003);
					...
					if(!requestString.isEmpty()) {
						...
						if(rCount == id || hash_map.containsKey(rCount)) {
							...
							if(requestString.substring(0, 3).equalsIgnoreCase("MTL")){
								mtlServer.sendRequestToFE("Already Executed");
							}
						}
						else if(rCount<id){
							...
							if(requestString.substring(0, 3).equalsIgnoreCase("MTL")){
								mtlServer.sendRequestToFE("Request will Execute Later");
							}
						}
		...
		Runnable t2 = () -> {
			...
			try {
				...
				while(true) {
					...
					System.out.println(requestString);
					System.out.println(requestString.substring(0, 3));
					if(requestString.substring(0, 3).equalsIgnoreCase("MTL")) {
						countErrorMTL++;
						System.out.println(countErrorMTL);
					}

					else if(requestString.substring(0, 3).equalsIgnoreCase("QUE")) {
						countErrorQUE++;
						System.out.println(countErrorQUE);
					}
					else if(requestString.substring(0, 3).equalsIgnoreCase("SHE")){
						countErrorSHE++;
						System.out.println(countErrorSHE);
					}
				}
			}
			catch(Exception e) {
	...
	public void send(String finalRequestString, String replicaName) {
		LOGGER.info("Request Recieved: "+ finalRequestString);

		System.out.println("MTL Count in Send "+countErrorMTL);
		System.out.println("QUE Count in Send "+countErrorQUE);
		System.out.println("SHE Count in Send "+countErrorSHE);
		if(countErrorMTL <= 2 ) {
			if (replicaName.substring(0, 3).equalsIgnoreCase("MTL")) {
				String replyToFE = mtlServerError.cancelAppointment(finalRequestString);
				mtlServer.sendRequestToFE(replyToFE);
				LOGGER.info("Error Replica Started: Montreal");
			}
		}
		if(countErrorQUE <= 2 ) {
			if (replicaName.substring(0, 3).equalsIgnoreCase("QUE")) {
				String replyToFE = queServerError.cancelAppointment(finalRequestString);
				queServer.sendRequestToFE(replyToFE);
				LOGGER.info("Error Replica Started: Quebec");
			}
		}
		if(countErrorSHE <= 2 ) {
			if (replicaName.substring(0, 3).equalsIgnoreCase("SHE")) {
				String replyToFE = sheServerError.cancelAppointment(finalRequestString);
				sheServer.sendRequestToFE(replyToFE);
				LOGGER.info("Error Replica Started: Sherbrooke");
			}
		}
		if(countErrorMTL >= 3 || countErrorQUE >= 3 || countErrorSHE >= 3) {
			System.out.println("Correct MTL must be called");
			mtlServerError = null;
			queServerError = null;
			sheServerError = null;

			System.out.println("Error Server Killed and Started the Correct Server");
			LOGGER.info("Error Replica Killed and Correct Replica is Running");
			String[] split = null;
			LOGGER.info("Request Recieved: "+ finalRequestString);
			...
			switch(replicaName.substring(3, 4)) {
				case "A" : {
					switch(menuNumber) {
						...
						case 3 : {
							LOGGER.info("Method Request by Admin: List Available Appointments");
							String appointmentType = split[1];
						...
						case 4 : {
							LOGGER.info("Method Requested by Admin: Book Appointment");
							String appointmentType = split[1];
							String appointmentID = split[2];
						...
						case 5 : {
							LOGGER.info("Method Requested by Admin: Get Appointment Schedule");
						...
						case 6:{
							LOGGER.info("Method Requested by Admin: Cancel Appointment");
							System.out.println("Replica Name "+ replicaName);
						...
						case 7:{
							LOGGER.info("Method Requested by Admin: Swap Appointment");
				...
				case "P" : {
					switch(menuNumber) {
						case 1 : {
							LOGGER.info("Method Request by Patient: Book Appointment");
							String appointmentType = split[1];
							String appointmentID = split[2];
							String patientID = split[3];
							String replyToFE = null;
						...
						case 3 : {
							LOGGER.info("Method Requested By Patient: Get Appointment Schedule");
							String patientID = split[1];
							if(replicaName.substring(0, 3).equalsIgnoreCase("MTL")) {
								String serverReplies = null;
								serverReplies = mtlServer.getAppointmentSchedule(patientID);
		...
		if(countErrorQUE >= 3) {
			if (replicaName.substring(0, 3).equalsIgnoreCase("QUE")) {
				String replyToFE = "Correct QUE must be called";
				queServer.sendRequestToFE(replyToFE);
			}
		}
		if(countErrorSHE >= 3) {
